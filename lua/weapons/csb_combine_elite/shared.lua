...
SWEP.Purpose				= "Kek"
SWEP.PrintName				= "Combine Elite [SUIT]"
SWEP.AbilityPrimaryDesc		= "[Hunter Visor]: A powerful, seethrough, life force scanner."
SWEP.AbilitySecondaryDesc	= "[Roller]: A deployable Roller Mine, remotely detonable."

if CLIENT then
SWEP.WepSelectIcon 		= surface.GetTextureID("vgui/entities/csb_combine_elite_icon.vtf")
SWEP.Slot				= 5
SWEP.SlotPos			= 5
SWEP.IconLetter			= "O"
...
SWEP.CSBPlyCombineEDetR = {
	sound = {"npc/combine_soldier/vo/boomer.wav",{"npc/combine_soldier/vo/bouncerbouncer.wav","npc/combine_soldier/vo/gosharp.wav","npc/combine_soldier/vo/gosharpgosharp.wav"},"npc/combine_soldier/vo/displace2.wav"},
	duration = {0,0,0},
	channel = CHAN_AUTO,
	volume = VOL_NORM,
	level = 75,
...
resource.AddFile( "materials/abilities/celite_ability_one.png" )
resource.AddFile( "materials/abilities/celite_ability_two.png" )

SWEP.PlyModel 					= "models/vjfix/combine_super_soldier.mdl"
SWEP.PlyHandsModel 				= "models/weapons/combine_arms/juonovom/hl2 vanilla/c_arms_combine_elite.mdl"
SWEP.NPCAnims					= true
...
SWEP.NotifyTab = {
	...
	['nottext'] = {
		...
		[6] = {
			['pos'] 	= {x = 150, y = 85},
			['size'] 	= {w = 188, h = 72},
			['text'] 	= "-Transhuman Elite Infantry-",
			['font'] 	= "CSB_Notify_Secondary",
			['color'] 	= Color(0, 255, 155),
		},
		[7] = {
		...
		[8] = {
			['pos'] 	= {x = 15, y = 150},
			['size'] 	= {w = 220, h = 72},
			['text'] 	= "-HUNTER VISOR- Highlights enemy vitals.",
			['font'] 	= "CSB_Notify_Secondary",
			['color'] 	= Color(0, 255, 155),
		},
		[9] = {
			['pos'] 	= {x = 15, y = 200},
			['size'] 	= {w = 220, h = 72},
			['text'] 	= "-ROLLER- Deploy remotely detonable Roller Mine.",
			['font'] 	= "CSB_Notify_Secondary",
			['color'] 	= Color(0, 255, 155),
		},
	},
...
SWEP.SuitAbilityPrimaryIcon 	= "materials/abilities/celite_ability_one.png"
SWEP.SuitAbilitySecondaryIcon 	= "materials/abilities/celite_ability_two.png"
...
self:SetNW2Float("CSB_PrimaryAbilityDuration", 15)
...
function SWEP:CSBHoloPlay() self:SetBodygroup(1,1) end
...
function SWEP:CSBPreSuitApply()
	...
	self.Owner:SetNW2Bool("CSB_CombineE_AbilityONE", false)
	--self.Owner:GetHands():SetSubMaterial( 0, "models/combine_soldier/combine_elite" )
...
function SWEP:CSBPrimaryAbility()
	...
	elseif self:GetNW2Bool("CSB_PrimaryAbility") == true and (self.Owner:GetNW2Bool("CSB_PlayingAnim") != true or (self.Owner:GetNW2Bool("CSB_PlayingAnim") == true and self.Owner:GetNW2Int("CSB_Gslot") != 6)) then
		...
		if self.Owner:GetNW2Bool("CSB_CombineE_AbilityONE") == true then return end
		
		self:SetNW2Bool("CSB_PrimaryAbility", false)
		self.Owner:SetNW2Bool("CSB_CombineE_AbilityONE", true)
		self.Owner:SendCSBAnimation(6, "csb_elite_ability_one", true, true, true, nil, nil, true, nil)
		
		self.Owner:CSBEmitSoundSequence(self.CSBPlyCombineEAOneA, true)
		
		self:EmitSound("CSBPlySound.CombineE.Item.AbilityONEActive")
		...
		if (not timer.Exists("CSB_PRIMARYAbilityActive" .. self.Owner:EntIndex() .. self:EntIndex())) then
			...
			timer.Create("CSB_PRIMARYAbilityActive" .. self.Owner:EntIndex() .. self:EntIndex(), self:GetNW2Float("CSB_PrimaryAbilityDuration"), 1, function()
				...
				self.Owner:SetNW2Bool("CSB_CombineE_AbilityONE", false)
				self:EmitSound("CSBPlySound.CombineE.Item.AbilityONEDeactive")
...
function SWEP:CSBSecondaryAbility()
	...
	if self:GetNW2Bool("CSB_SecondaryAbility") == false and (self.Owner:GetNW2Bool("CSB_PlayingAnim") != true or (self.Owner:GetNW2Bool("CSB_PlayingAnim") == true and self.Owner:GetNW2Int("CSB_Gslot") != 6)) then
		...
		if IsValid(self.roller) then
		
			self.roller:Destroy()
			
			self.Owner:CSBEmitSoundSequence(self.CSBPlyCombineEDetR, true)
	...
	elseif self:GetNW2Bool("CSB_SecondaryAbility") == true and (self.Owner:GetNW2Bool("CSB_PlayingAnim") != true or (self.Owner:GetNW2Bool("CSB_PlayingAnim") == true and self.Owner:GetNW2Int("CSB_Gslot") != 6)) then
		...
		if (SERVER) then
			...
			timer.Simple(0.8, function()
				...
				if !IsValid(self) or !IsValid(self.Owner) or IsValid(self.roller) then return end
				self.roller = ents.Create( "sent_ability_roller" )
				local plyAng = Angle(0, self.Owner:GetAngles().y, 0)
				local offsetVec = Vector(20, 0, 20)
				local offsetAng = Angle(0, 0, 0)
				local newPos, newAng = LocalToWorld(offsetVec, offsetAng, self.Owner:GetPos(), plyAng)
				self.roller:SetPos(newPos)
				self.roller:SetAngles(newAng)
				self.roller:SetOwner(self.Owner)
				self.roller:Spawn()
				self.roller:Activate()
				self.roller.ent:SetNW2Bool("CSB_SpawnedEQ", true)
				self.roller.ent:SetOwner(self.Owner)
				self.roller.ent:SetName(self.Owner:Nick() .. "'s Roller")
				self:DeleteOnRemove(self.roller)
				self.roller.ent:CSB_NPCFollowPlayer(self.Owner, 100, 200, 1000, 1000)

			end)
			
		end
		
		self.Owner:SendCSBAnimation(6, "grenPlace", true, true, true, nil, nil, true, nil)
		
		local delay = self.Owner:SequenceDuration(self.Owner:LookupSequence("grenPlace"))
		
		if (SERVER) then self.Owner:CSBDrawProp("models/Roller.mdl", nil, 0.2, 0, "ValveBiped.Bip01_L_Hand", delay-0.55, Vector(3,-3,0), Angle(0,0,0)) end
		
		self.Owner:CSBEmitSoundSequence(self.CSBPlyCombineEDR, true)
...
function SWEP:CSBOnDrop()
	...
	if IsValid(ply) then
		...
		ply:SetNW2Bool("CSB_CombineE_AbilityONE", false)
		
		--if IsValid(ply:GetHands()) then ply:GetHands():SetSubMaterial( 0, "" ) end
...
function SWEP:CSBOnRemove()
	...
	if IsValid(self.Owner) then
		...
		--if IsValid(self.Owner:GetHands()) then self.Owner:GetHands():SetSubMaterial( 0, "" ) end
...
RegisterLuaAnimation('csb_elite_ability_one', {
	FrameData = {
		{
			BoneInfo = {
...
SWEP.HUDTab = {
	['health'] = {
		...
		['bghpsize'] 		= {w = 285, h = 60}, -- Health background size.
		['bghpround'] 		= 8, -- Health background rounded corners.
		['bghpcolor'] 		= Color(0, 0, 0, 155), -- Health background color.
		--
		['nhppos'] 			= {x = 185, y = 952}, -- Health vaule pos.
		['nhpfont'] 		= "csb_font_playerinfo", -- Health vaule font.
		['nhpcolor'] 		= Color(0,255,155,255), -- Health vaule color.
		['nhplimit'] 		= 999, -- Health vaule limit.
		--
		...
		['thpcolor'] 		= Color(0,255,155,255), -- Health text color.
		--
		['nmhppos'] 		= {x = 285, y = 940.5}, -- Max health value pos.
		['nmhpfont'] 		= "csb_font_ammoinfo2", -- Max health value font.
		['nmhpcolor'] 		= Color(0,255,155,255), -- Max health value color.
		['nmhplimit'] 		= 999, -- Max health value limit.
		--
		['tmhppos'] 		= {x = 285, y = 967}, -- Max health text pos.
		['tmhptext'] 		= "MAX", -- Max health text.
		['tmhpfont'] 		= "csb_font_barinfo", -- Max health text font.
		['tmhpcolor'] 		= Color(0,255,155,255), -- Max health text color.
	},
	['armor'] = {
		...
		['bgarmrsize'] 		= {w = 285, h = 60}, -- Armor background size.
		['bgarmrround']		= 8, -- Armor background rounded corners.
		['bgarmrcolor'] 	= Color(0, 0, 0, 155), -- Armor background color.
		--
		['narmrpos'] 		= {x = 185, y = 1022}, -- Armor value pos.
		['narmrfont'] 		= "csb_font_playerinfo", -- Armor value font.
		['narmrcolor'] 		= Color(0,255,155,255), -- Armor value color.
		['narmrlimit'] 		= 999, -- Armor value limit.
		--
		['tarmrpos'] 		= {x = 80, y = 1037}, -- Armor text pos.
		['tarmrtext'] 		= "ENERGY", -- Armor text.
		['tarmrfont'] 		= "csb_font_barinfo", -- Armor text font.
		['tarmrcolor'] 		= Color(0,255,155,255), -- Armor text color.
		--
		['nmarmrpos'] 		= {x = 285, y = 1010}, -- Max armor value pos.
		['nmarmrfont'] 		= "csb_font_ammoinfo2", -- Max armor value font.
		['nmarmrcolor'] 	= Color(0,255,155,255), -- Max armor value color.
		['nmarmrlimit'] 	= 999, -- Max armor value limit.
		--
		['tmarmrpos'] 		= {x = 285, y = 1037}, -- Max armor text pos.
		['tmarmrtext'] 		= "MAX", -- Max armor text.
		['tmarmrfont'] 		= "csb_font_barinfo", -- Max armor text font.
		['tmarmrcolor'] 	= Color(0,255,155,255), -- Max armor text color.
	},
	['weaponname'] = {
		['bgwepnpos'] 		= {x = 1598, y = 922}, -- Weapon background pos.
		['bgwepnsize'] 		= {w = 280, h = 25}, -- Weapon background size.
		['bgwepnround']		= 8, -- Weapon background rounded corners.
		['bgwepncolor'] 		= Color(0, 0, 0, 155), -- Weapon background color.
		--
		['twepnpos'] 		= {x = 1738, y = 933.5}, -- Weapon name text (curently held) pos.
		['twepnfont'] 		= "csb_font_barinfo", -- Weapon name text (curently held) font.
		['twepncolor'] 		= Color(0,255,155,255), -- Weapon name text (curently held) color.
	},
	['weaponstat'] = {
		['bgweppos'] 		= {x = 1598, y = 957}, -- Weapon stats background pos.
		['bgwepsize'] 		= {w = 280, h = 95}, -- Weapon stats background size.
		['bgwepround']		= 8, -- Weapon stats background rounded corners.
		['bgwepcolor'] 		= Color(0, 0, 0, 155), -- Weapon stats background color.
		--
		['n1weppos'] 		= {x = 1722, y = 1022}, -- Weapon stats clip value pos.
		['n1wepfont'] 		= "csb_font_ammoinfo", -- Weapon stats clip value font.
		['n1wepcolor'] 		= Color(0,255,155,255), -- Weapon stats clip value color.
		['n1weplimit'] 		= 999, -- Weapon stats clip value limit.
		--
		['n2weppos'] 		= {x = 1833, y = 1017}, -- Weapon stats reserve ammo value pos.
		['n2wepfont'] 		= "csb_font_ammoinfo2", -- Weapon stats reserve ammo value font.
		['n2wepcolor'] 		= Color(0,255,155,255), -- Weapon stats reserve ammo value color.
		['n2weplimit'] 		= 999, -- Weapon stats reserve ammo value limit.
		--
		['n3weppos'] 		= {x = 1833, y = 973}, -- Weapon stats secondary ammo value pos.
		['n3wepfont'] 		= "csb_font_ammoinfo2", -- Weapon stats secondary ammo value font.
		['n3wepcolor'] 		= Color(0,255,155,255), -- Weapon stats secondary ammo value color.
		['n3weplimit'] 		= 999, -- Weapon stats secondary ammo value limit.
		--
		['dmgweppos'] 		= {x = 1833, y = 970}, -- Weapon stats damage value pos.
		['dmgwepfont'] 		= "csb_font_ammoinfo2", -- Weapon stats damage value font.
		['dmgwepcolor'] 	= Color(0,255,155,255), -- Weapon stats damage value color.
		['dmgweplimit'] 	= 999, -- Weapon stats damage value limit.
		--
		['tweppos'] 		= {x = 1627, y = 1037}, -- Weapon stats ammo text pos.
		['tweptext'] 		= "AMMO", -- Weapon stats ammo text.
		['twepfont'] 		= "csb_font_barinfo", -- Weapon stats ammo text font.
		['twepcolor'] 		= Color(0,255,155,255), -- Weapon stats ammo text color.
		--
		['tpaweppos'] 		= {x = 1833, y = 1037}, -- Weapon stats reserve ammo text pos.
		['tpaweptext'] 		= "RESERVE", -- Weapon stats reserve ammo text.
		['tpawepfont'] 		= "csb_font_barinfo", -- Weapon stats reserve ammo text font.
		['tpawepcolor'] 	= Color(0,255,155,255), -- Weapon stats reserve ammo text color.
		--
		['tspweppos'] 		= {x = 1833, y = 993}, -- Weapon stats secondary ammo text pos.
		['tspweptext'] 		= "SPECIAL", -- Weapon stats secondary ammo text.
		['tspwepfont'] 		= "csb_font_barinfo", -- Weapon stats secondary ammo text font.
		['tspwepcolor'] 	= Color(0,255,155,255), -- Weapon stats secondary ammo text color.
		--
