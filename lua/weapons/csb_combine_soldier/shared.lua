...
SWEP.Purpose				= "Kek"
SWEP.PrintName				= "Combine Soldier [SUIT]"
SWEP.AbilityPrimaryDesc		= "[Tigh Grip]: Increased accuracy for a limited time."
SWEP.AbilitySecondaryDesc	= "[Riot Cover]: A deployable shield, careful as it can stand only so much."

if CLIENT then
SWEP.WepSelectIcon 		= surface.GetTextureID("vgui/entities/csb_combine_soldier_icon.vtf")
SWEP.Slot				= 5
SWEP.SlotPos			= 5
SWEP.IconLetter			= "O"
...
sound.Add( {
	name = "CSBPlySound.CombineS.AbilityONEActive",
	channel = CHAN_AUTO,
	volume = VOL_NORM,
	level = 75,
	pitch = 80,
	sound = { "csb/abilities/csoldier_ab_one_a3.wav" }
} )

sound.Add( {
	name = "CSBPlySound.CombineS.AbilityONEDeactive",
	channel = CHAN_AUTO,
	volume = VOL_NORM,
	level = 75,
	pitch = 80,
	sound = { "csb/abilities/csoldier_ab_one_d3.wav" }
} )
...
SWEP.CSBPlyCombineSDC = {
sound = {"npc/combine_soldier/vo/team.wav","npc/combine_soldier/vo/cover.wav",{"npc/combine_soldier/vo/fullactive.wav","npc/combine_soldier/vo/delivered.wav","npc/combine_soldier/vo/secure.wav"}},
duration = {0,0,0},
channel = CHAN_AUTO,
volume = VOL_NORM,
level = 75,
...
resource.AddFile( "materials/abilities/csoldier_ability_one.png" )
resource.AddFile( "materials/abilities/csoldier_ability_two.png" )

SWEP.PlyModel 					= "models/vjfix/combine_soldier.mdl"
SWEP.PlyBodyGroups 				= nil 
SWEP.PlySkin		 			= 0	
SWEP.PlySubMaterials 			= nil
SWEP.PlyHandsModel 				= "models/weapons/combine_arms/juonovom/hl2 vanilla/c_arms_combine_regular.mdl"
SWEP.NPCAnims					= true
...
SWEP.NotifyTab = {
	...
	['nottext'] = {
		...
		[8] = {
			['pos'] 	= {x = 15, y = 150},
			['size'] 	= {w = 220, h = 72},
			['text'] 	= "-TIGHT GRIP- Increased accuracy with all weapon types.",
			['font'] 	= "CSB_Notify_Secondary",
			['color'] 	= Color(0, 255, 155),
		},
		[9] = {
			['pos'] 	= {x = 15, y = 200},
			['size'] 	= {w = 220, h = 72},
			['text'] 	= "-RIOT COVER- Deployable cover with a limited durability.",
			['font'] 	= "CSB_Notify_Secondary",
			['color'] 	= Color(0, 255, 155),
		},
	},
...
SWEP.SuitAbilityPrimaryIcon 	= "materials/abilities/csoldier_ability_one.png"
SWEP.SuitAbilitySecondaryIcon 	= "materials/abilities/csoldier_ability_two.png"
...
function SWEP:CSBPrimaryAbility()
	...
	elseif self:GetNW2Bool("CSB_PrimaryAbility") == true and (self.Owner:GetNW2Bool("CSB_PlayingAnim") != true or (self.Owner:GetNW2Bool("CSB_PlayingAnim") == true and self.Owner:GetNW2Int("CSB_Gslot") != 6)) then
		...
		self.Owner:SendCSBAnimation(6, "console_type_exit", true, true, true, nil, nil, true, nil)
		self:EmitSound("CSBPlySound.CombineS.AbilityONEActive")
		
		self.Owner:SetNW2Bool("CSB_CombineS_AbilityONE", true)
		...
		if (not timer.Exists("CSB_PRIMARYAbilityActive" .. self.Owner:EntIndex() .. self:EntIndex())) then
			...
			timer.Create("CSB_PRIMARYAbilityActive" .. self.Owner:EntIndex() .. self:EntIndex(), self:GetNW2Float("CSB_PrimaryAbilityDuration"), 1, function()
				...
				if !IsValid(self) or !IsValid(self.Owner) then return end
					...
					self:EmitSound("CSBPlySound.CombineS.AbilityONEDeactive")
					self.Owner:SetNW2Bool("CSB_CombineS_AbilityONE", false)
...
function SWEP:CSBSecondaryAbility()
	...
	elseif self:GetNW2Bool("CSB_SecondaryAbility") == true and (self.Owner:GetNW2Bool("CSB_PlayingAnim") != true or (self.Owner:GetNW2Bool("CSB_PlayingAnim") == true and self.Owner:GetNW2Int("CSB_Gslot") != 6)) then
		...
		if (SERVER) then 
			...
			self:CSBSecondaryIconColor(self.Owner, Color(0, 135, 80, 255))
		
			timer.Simple(0.8, function()
			
				if !IsValid(self) or !IsValid(self.Owner) or IsValid(self.cower) then return end
			
				self.cower = ents.Create( "sent_ability_cower" )
				...
				self.cower:SetPos(newPos)
				self.cower:SetAngles(newAng)
				self.cower:Spawn()
				self.cower:Activate()
				self.cower:SetOwner(self.Owner)
				--self.cower:SetModel("models/Items/combine_rifle_ammo01.mdl")
				self:DeleteOnRemove(self.cower)
		...
		if (SERVER) then self.Owner:CSBDrawProp("models/props_combine/combine_barricade_med02c.mdl", nil, 0.05, 0, "ValveBiped.Bip01_L_Hand", delay-0.55, Vector(1,-2,0), Angle(0,0,90)) end
		
		self.Owner:CSBEmitSoundSequence(self.CSBPlyCombineSDC, true)
...
function SWEP:CSBOnDrop()
	...
	if IsValid(ply) then
		...
		ply:SetNW2Bool("CSB_CombineS_AbilityONE", false)
